...
export function applyUnlock(node) {
  ...
  switch (u) {
    ...
    case 'butcher': {
      state.unlockedTowers?.add('butcher');
      break;
    }
    case 'steam_age': {
      state.age = 'steam';
      for (const k of ['furnace','water_pump','pipe','steam_boiler','steam_engine','pulley','belt','tank','inline_pump']) state.unlockedTowers?.add(k);
